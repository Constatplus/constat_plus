...
import '../../infrastructure/payments/apple_payment_provider.dart';
...
class AppleBillingController {
  AppleBillingController({
    ApplePaymentProvider? provider,
    SupabaseProviderProductRepository? productRepository,
  }) : _provider = provider ?? ApplePaymentProvider.instance,
       _productRepository =
           productRepository ?? SupabaseProviderProductRepository();

  final ApplePaymentProvider _provider;
  ...
  Future<BillingProduct> loadProduct(SubscriptionPlan plan) async {
    final mapping = await _productRepository.findAppleProduct(plan.code);
    if (mapping == null) {
      throw StateError(
        'Aucun produit Apple actif n’est associé à ${plan.name}.',
      );
    }
    final products = await _provider.loadProducts({mapping.providerProductId});
    if (products.isEmpty) {
      throw StateError(
        'Le produit ${mapping.providerProductId} est introuvable dans l’App Store.',
      );
    }
  ...
  Future<PurchaseResult> purchase(
    ...
    final request = PurchaseRequest(
      ...
      missionId: missionId,
      idempotencyKey:
          'apple:${user.id}:${DateTime.now().microsecondsSinceEpoch}',
    );
