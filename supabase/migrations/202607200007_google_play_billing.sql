...
create unique index one_time_purchases_google_token_unique
  on public.one_time_purchases (provider, provider_purchase_token)
...
values
  ('mission_unit', 'google_play', 'constat_plus_mission_unit', 'android', true),
  ('solo', 'google_play', 'constat_plus_solo_monthly', 'android', true),
  ('pro', 'google_play', 'constat_plus_pro_monthly', 'android', true)
...
begin
  ...
  where product.provider = 'google_play'
    and product.platform = 'android'
  ...
  if (v_billing_period = 'monthly' and p_product_kind <> 'subscription')
    ...
    raise exception 'google_play_product_kind_mismatch';
  end if;
  ...
  if v_user_id is null then
    if p_product_kind = 'subscription' then
      ...
      where provider = 'google_play'
        and provider_purchase_token = p_purchase_token;
    else
      select user_id into v_user_id
  ...
  insert into public.payment_events (
    ...
    'google_play',
    'purchase:' || p_payload_hash,
    'google_play_' || p_product_kind || '_' || p_status,
    p_payload_hash,
    now()
...
revoke all on function public.record_google_play_purchase(
  uuid, text, text, text, text, text,
  timestamptz, timestamptz, boolean, text, uuid
) from public;
grant execute on function public.record_google_play_purchase(
  uuid, text, text, text, text, text,
  timestamptz, timestamptz, boolean, text, uuid
) to service_role;

comment on function public.record_google_play_purchase(
  uuid, text, text, text, text, text,
  timestamptz, timestamptz, boolean, text, uuid
) is 'Applique uniquement un achat préalablement vérifié auprès de Google Play.';
